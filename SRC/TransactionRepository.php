...
class TransactionRepository {
    ...
    public function findAll() : array {
        return $this->pdo->query("select * from transactions order by created_at desc")
        ->fetchAll();
    }
    ...
    try {
        ...
        $stmt= $this -> pdo->prepare("update accounts set balance = balnce + :amount where id= :id");
        $stmt-> execute(['amount' => $amount, 'id' => $account_id]);


        $stmt = $this -> pdo->prepare("Insert into transaction (to_account_id, typ, amount) values (?, 'deposit', ?)");
   ...
   public function withdraw (int $account_id, float $amount): void {
    ...
    try {
        ...
        $stmt = $this->pdo->prepare("insert into transactions (from_account_id, typ, amount) values (?, 'withdrawal', ?)");
        $stmt->execute([$account_id, $amount]);
        $this->pdo->commit();
    } catch (Exception $e) {
        $this->pdo->rollBack();
    ...
    public function transfer (int $from_account_id, int $to_account_id, float $amount): void {
        ...
        try {
            ...
            $stmt = $this->pdo->prepare("insert into transactions (from_account_id, to_account_id, typ, amount) values (?, ?, 'transfer', ?)");
